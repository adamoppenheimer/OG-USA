...
#%%
#get_pop_objs(5, 20, 100, 0, 100, 2019, True)
#get_pop_objs(20, 80, 100, 0, 100, 2014, True)
#%%
def get_fert(totpers, min_yr, max_yr, graph=False):
    ...
    age_midp       = (13,) vector, midpoint age of age bins ranges from
                     ...
                     first two and last two are not data. ### first 2 and last 2 are single, because added want interpolation to work nicely (take anything in between as fertility rate from interpolation)
    ...
    fert_filename = cur_path + "/data/demographic/Fertility.csv"
    ...
    #fert_func = si.interp1d(fert_data["Age"], fert_data["Values"],\
    #    bounds_error=False,\
    #    kind='cubic')  # this is the interpolation
    ...
    #pop_func = si.interp1d(ages, curr_pop_pct, kind='cubic')
    pop_func = si.splrep(ages, curr_pop_pct)
    new_bins = np.linspace(min_yr, max_yr,\
                            num_sub_bins * max_yr)
    #print("New bins: ", new_bins.shape)
    #curr_pop_sub = pop_func(new_bins)
    curr_pop_sub = si.splev(new_bins, pop_func)
    curr_pop_sub = curr_pop_sub / curr_pop_sub.sum()
    #print("Sum: ", curr_pop_sub.sum())
    #print("Curr pop sub: ", curr_pop_sub.shape)
    ...
    #fert_rates_sub[pred_ind] = np.float64(fert_func(age_pred))  # does interpolation and finds fert_rates in the valid age range
...
def select_data(data):
        ...
        return new_data
#%%
#get_mort(10, 0, 100, graph=True)
#%%
def get_mort(totpers, min_yr, max_yr, graph=False):
    '''
    # mortality rate is cumulative
    --------------------------------------------------------------------
    ...
    #mort_func = si.interp1d(mort["Age"] + 1, mort_rates_all, kind='cubic', bounds_error=False)
    mort_func = si.splrep(mort["Age"] + 1, mort_rates_all)
    #print(mort_rates_all)
    #print("Len subbins: ", len_subbins)

    new_bins = np.linspace(min_yr, max_yr,\
                            len_subbins * totpers, dtype=float)

    #print("New bins shape: ", new_bins.shape)

    mort_rates_sub_orig = 1 - si.splev(new_bins, mort_func)
    mort_rates_sub_orig[mort_rates_sub_orig > 1] = 1
    #mort_rates_sub_orig = np.ma.array(mort_rates_sub_orig,\
    #                        mask=np.isnan(mort_rates_sub_orig))
    ...
    for i in range(min_yr, max_yr + 1):
        ...
        #print("Beginning bin: ", beg_sub_bin)
        #print("End bin: ", end_sub_bin)
        tot_period_surv = (np.log(mort_rates_sub_orig[beg_sub_bin:end_sub_bin]) ).sum()
        end_surv = np.log(1 - mort_rates_all.iloc[i])
        #print("Age box: ", int(i * max_yr / (totpers - 1)))
        pow = end_surv / tot_period_surv
        mort_rates_sub[beg_sub_bin:end_sub_bin] = mort_rates_sub_orig[beg_sub_bin:end_sub_bin] ** pow
    #print("Last bin: ", end_sub_bin)
    #print("Len subbins: ", len_subbins)
    #print("mort_rates_sub_orig shape: ", new_bins.shape)
    #print("Min year: ", min_yr)
    #print("Max year: ", max_yr)
    #print("Total periods: ", totpers)

    #for i in range(totpers - 1):
    #    beg_sub_bin = int(end_sub_bin)
    #    end_sub_bin = int(np.rint((i + 1) * len_subbins))
    #    tot_period_surv = np.log(mort_rates_sub_orig[beg_sub_bin:end_sub_bin]).sum()
    #    end_surv = np.log(mort_rates_sub_orig[end_sub_bin])
    #    pow = end_surv / tot_period_surv
    #    mort_rates_sub[beg_sub_bin:end_sub_bin] = mort_rates_sub_orig[beg_sub_bin:end_sub_bin] ** pow

    #mort_rates_sub = (1 - mort_func(new_bins) ) ** (1 / totpers)
...
def get_mort(totpers, min_yr, max_yr, graph=False):
    ...
    mort_filename = cur_path + "/Data/Mortality.csv"

    mort = pd.read_csv(mort_filename, sep="\s+", usecols=["Year", "Age", "Total"])
    mort_data = select_data(mort)[:110] # because don't know how to deal with 110

    pop_filename = cur_path + "/Data/Population.csv"
    pop = pd.read_csv(pop_filename, sep="\s+", usecols=["Year", "Age", "Total"])
    pop = select_data(pop)[:110]
    mort_rates_all = mort_data['Total'] / pop['Total']

    age_year_all = mort_data['Age'] + 1

    # Calculate implied mortality rates in sub-bins of mort_rates_all.
    mort_rates_mxyr = mort_data[0:max_yr]["Total"] / pop[0:max_yr]["Total"]
    num_sub_bins = int(10000)
    len_subbins = ((np.float64((max_yr - min_yr + 1) * num_sub_bins)) /
                   totpers)
    ...
    for i in range(max_yr):
        mort_rates_sub[i * num_sub_bins:(i + 1) * num_sub_bins] =\
            (1 - ((1 - mort_rates_mxyr.iloc[i]) ** (1.0 / num_sub_bins)))
    mort_rates = np.zeros(totpers)
    end_sub_bin = 0
    for i in range(totpers):
        beg_sub_bin = int(end_sub_bin)
...
def pop_rebin(curr_pop_dist, totpers_new):
    ...
    return curr_pop_new
###DR. EVAN'S CODE ABOVE
"""
#%%
def pop_rebin(curr_pop_dist, totpers_new):
    '''
    # using population data to calculate immigration rates as residuals.
    # popu,lation by age in year and model period is less than population by age data
    # combine population bin as 1. Separate population into percentage bins.
    --------------------------------------------------------------------
    For cases in which totpers (E+S) is less than the number of periods
    in the population distribution data, this function calculates a new
    population distribution vector with totpers (E+S) elements.
    ...
    '''
    totpers_orig = len(curr_pop_dist)
    print("New periods: ", totpers_new)
    print("Orig periods: ", totpers_orig)
    if int(totpers_new) == totpers_orig:
        curr_pop_new = curr_pop_dist
    elif int(totpers_new) < totpers_orig:
        num_sub_bins = float(10000)
        ages = np.linspace(0, totpers_orig - 1, totpers_orig)
        print("totpers orig: ", curr_pop_dist.shape)
        print("ages: ", ages.shape)
        #pop_func = si.interp1d(ages, curr_pop_dist, kind='cubic')
        pop_func = si.splrep(ages, curr_pop_dist)
        new_bins = np.linspace(0, totpers_orig - 1,\
                                num_sub_bins * totpers_orig)
        #pop_ests = pop_func(new_bins)
...
def get_imm_resid(totpers, min_yr, max_yr, graph=True):
    ...
    Assume constant by age.
    considers each individual as hosehold asexual born baby
    no gender
    kids not associated with household
    --------------------------------------------------------------------
...
def immsolve(imm_rates, *args):
    '''
    fix population after certain time, what immigration rate choose to make
    popukation constant
    --------------------------------------------------------------------
...
def get_pop_objs(E, S, T, min_yr, max_yr, curr_year, GraphDiag=True):
    '''
    gets all the data and outputs all needed for ogusa model.
    --------------------------------------------------------------------
    ...
    pop_data_samp = pop_data[(pop_data['Age'] >= min_yr - 1) &
                             (pop_data['Age'] <= max_yr - 1)]
    #pop_2013 = np.array(pop_data_samp['2017'], dtype='f')
